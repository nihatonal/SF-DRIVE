...
function DropzoneComponent(props) {
  ...
  const thumbs = files.map(file => (
    ...
    <div className={`preview-wrapper ${props.classPreview}`} key={uuidv4()}>
        ...
        <div className={`preview-img-wrapper mobile-img-wrapper ${props.classPreviewImg}`}>
        <img src={file.preview} alt={file.name} />  
          {!props.loadingFilter &&
          <div className='loading-filter'>

            {props.isLoading && <div className="img_icon-wrapper-drop" onClick={cancelHandler}>
              <CircularStaticSingle progress={props.percentage}/>
              <img src={X} className="photo-circle-text" alt="x icon" onClick={props.Cancel}/>
            </div>}

            <div className={!props.showRenew ? "input-hidden" : "img_icon-wrapper-drop" }>
        ...
        <div className={`preview-info ${props.classPrewiewinfo}`}>
            <div>
                <p className={`preview-info-name ${props.classPrewiewname}`}>{file.name.slice( 0, -4 )}</p>
                <p className={`preview-info-desc ${props.classPrewiewdesc}`}>{formatBytes(file.size)}, {file.type.slice(6)}</p>
            </div>
            <div onClick={() => deleteHandler(file.name)}>
              <img src={trash_blue} alt="trash"  
  ...
  return (
    <section className={`dropzone-container`} style={files.length > 0 ? {marginBottom:'40px'} : null}>
       
            <div {...getRootProps({style})} className={!showDropzone ? `dropzone-wrapper ${props.dropclassName}` : `dropzone-wrapper small ${props.dropclassName}` }>
                <input {...getInputProps()} id={props.id} name={props.name}/>
                
                <div className={`dropzone-content ${props.dropcontentclass}`}>
                    <img src={cloud}/>
                    {!showDropzone ? <p className={'input-desc'}>Перетащите или <span>выберите файл</span></p> :
